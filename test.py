...
def orderbook(args):
    try:
        ...
        if exec_qty == False:
            ...
            positionDB.updatePid(pid, current_pid)
...
def check_pid_and_restart():
    while True:
        pids = psutil.pids()
        orders = positionDB.getAllPending()
        for order in orders:
            if order['PID'] not in pids:
...
if __name__ == "__main__":
   args = {
       ...
       "c_ct": 2,
       "price": 0.00000933,
       "stop_loss": 0.00000900,
       "take_profit": 0.00001000,
       "timeframe": "15m",
       "strategy": "prova",
       "open": 1,
       "close": 2,
